...
def optimalZeropad(x, fs, f):
    ...
    #limit =  abs(fs/f - np.size(x))

    width = np.size(x)  ## starting width of signal. This will be increased each iteraton
    ...
    halfX[halfX<1e-6]=1e-6 ##replace 'zero values with 1e-6

    mX = 20 * np.log10(abs(halfX))
    
    cnt = np.size(mX[mX>-120])

    while cnt > 1  and cnt < 20:

        width += 1

        fftBuffer = createZeropadFFTBuffer(x, width)
        ...
        halfX[halfX<1e-6]=1e-12 ##replace 'zero values with 1e-12
    ...
    #mX=20*np.log10(
    return cnt, M, np.array(mX[:(np.size(mX)/2) + 1])
