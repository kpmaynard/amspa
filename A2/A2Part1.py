...
def genSine(A, f, phi, fs, t):
    """
    Inputs:
        A (float) =  amplitude of the sinusoid (1)
        f (float) = frequency of the sinusoid in Hz (10)
        phi (float) = initial phase of the sinusoid in radians (1)
        fs (float) = sampling frequency of the sinusoid in Hz (50)
        t (float) =  duration of the sinusoid (is second) (.1)
    Output:
    ...
    y=np.array([])
    N= fs * t
    y = A * np.cos( (2* np.pi*f* np.arange(N)/fs) + phi)
    return (y)
